// ARCHIVED: Legacy "Mallar" admin screen.
// Kept for reference while the app is rebuilt so template admin lives under Kontrolltyper.
...
import { CompanyHeaderLogo } from '../components/HeaderComponents';
...
export default function ManageTemplates({ route, navigation }) {
  ...
  const [modalFolderName, setModalFolderName] = useState(null);
  ...
  // Route-param sync (used when navigating here from the new 3-column control type view)
  useEffect(() => {
    try {
      const cid = String(route?.params?.companyId || '').trim();
      if (cid) setCompanyId(cid);
      const ct = String(route?.params?.controlType || '').trim();
      if (ct) setControlType(ct);
    } catch (_e) {}
  }, [route?.params?.companyId, route?.params?.controlType]);

  useEffect(() => {
    try {
      if (!route?.params?.openTemplateModal) return;
      const cid = String(route?.params?.companyId || companyId || '').trim();
      const ct = String(route?.params?.controlType || controlType || '').trim();
      ...
      // Create new template (from 3-column view)
      ...
      try {
        navigation.setParams({ openTemplateModal: false });
      } catch (_e) {}
    } catch (_e) {}
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [route?.params?.openTemplateModal]);
  ...
  const handleAddTemplate = async (overrideControlType) => {
    ...
    try {
      await createCompanyMall(
        {
          ...
          controlType: ct,
          folderId: modalFolderId || null,
          folderName: modalFolderName || null,
      ...
      try {
        const folderLabel = modalFolderName ? ` / ${modalFolderName}` : '';
        Alert.alert('Mall skapad', `Mallen har skapats under "${ct}${folderLabel}".`);
      } catch (_e) {}
    } catch (e) {
          ...
          onSelectProject={(payload) => {
            try {
              ...
              if (payload?.openTemplateModal && cid && payload?.controlType) {
                ...
                const folderName = payload?.folderName ?? null;
                ...
                if (tpl) {
                  ...
                  setModalFolderId(tpl?.folderId ?? folderId);
                  setModalFolderName(tpl?.folderName ?? folderName);
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
                {Platform.OS === 'web' ? (
                  <WebBreadcrumbHeader
                    ...
                    iconColor="#00897B"
                    extraLabel={previewTemplate ? `Förhandsvisning: ${previewTemplate.title || 'Namnlös mall'}` : ''}
                    onPressLabel={previewTemplate ? (() => setPreviewTemplate(null)) : undefined}
                  />
                ) : (
                  <>
                    <TouchableOpacity
              ...
              <View style={{ maxWidth: 680 }}>
                ...
                <View style={{ marginTop: 18, width: '84%', marginLeft: '8%' }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', flexShrink: 1 }}>
                      {(() => {
                        ...
                        return (
                          ...
                          >
                            <Ionicons name={ctMeta.icon || 'document-text-outline'} size={14} color="#fff" />
                          </View>
                        );
                      })()}
                      <Text style={{ fontSize: 13, fontWeight: '600', color: '#222', flexShrink: 1 }} numberOfLines={1}>
                        {controlType || 'Välj kontrolltyp i listan till vänster'}
                      </Text>
                    </View>
                    <Text style={{ fontSize: 12, color: '#777' }}>Styrs av sidomenyn</Text>
                  </View>
                  ...
                  {!String(controlType || '').trim() && hasSelectedCompany && (
                    <Text style={{ fontSize: 12, color: '#D32F2F', marginBottom: 4 }}>
                      Välj först en kontrolltyp i listan till vänster.
                    </Text>
                  )}
                  {!hasSelectedCompany && (
                    <Text style={{ fontSize: 12, color: '#D32F2F', marginBottom: 4 }}>
                ...
                {!typePickerOpen && !previewTemplate && (
                  <View style={{ marginTop: 22, width: '92%', marginLeft: '8%' }}>
                    ...
                    {hasSelectedCompany && !String(controlType || '').trim() && templates.length > 0 && (
                      <Text style={{ fontSize: 12, color: '#777', marginBottom: 4 }}>
                        Välj en kontrolltyp i listan till vänster för att filtrera listan.
                      </Text>
                    )}
                    {(() => {
                      const ct = String(controlType || '').trim();
              ...
              <View style={{ marginBottom: 10 }}>
                ...
                >
                  <View style={{ flexDirection: 'row', alignItems: 'center', flexShrink: 1 }}>
                    ...
                    <Text style={{ fontSize: 13, fontWeight: '600', color: '#222', flexShrink: 1 }} numberOfLines={1}>
                      {modalControlType || controlType || 'Välj kontrolltyp i listan till vänster'}
                    </Text>
                  </View>
                  <Text style={{ fontSize: 12, color: '#777' }}>Styrs av vyn</Text>
                </View>

                {(() => {
                  const folderLabel = modalFolderName
                    ? String(modalFolderName)
                    : (modalFolderId ? 'Vald mapp' : 'Utan mapp');
                  return (
                    <Text style={{ fontSize: 12, color: '#777', marginTop: 6 }} numberOfLines={1}>
                      Mapp: {folderLabel}
                    </Text>
                  );
                })()}
