...
import { Alert, Dimensions, Image, InteractionManager, Keyboard, KeyboardAvoidingView, LayoutAnimation, Modal, PanResponder, Platform, Pressable, ScrollView, Text, TextInput, TouchableOpacity, UIManager, useWindowDimensions, View } from 'react-native';
...
export default function BaseControlForm({
  ...
  const [, setSignerKeyboardShift] = useState(0);
  ...
  const [qualityDesc] = useState(initialValues.qualityDesc || '');
  const [coverageDesc] = useState(initialValues.coverageDesc || '');
  ...
  // ...existing code...
  const emptyRouteParamsRef = useRef({});
  const routeParams = route && route.params ? route.params : emptyRouteParamsRef.current;
  ...
  const initialValuesDate = initialValues?.date;
  ...
  useEffect(() => {
    ...
    const fallback = date || initialValuesDate || todayIso;
    setDateValue(fallback);
  }, [dateValue, date, initialValuesDate, todayIso]);
  const [deliveryDesc, setDeliveryDesc] = useState((initialValues && initialValues.deliveryDesc) || '');
  const [generalNote] = useState((initialValues && initialValues.generalNote) || '');
  ...
  }, [missingFields, controlType, participantsLabel, localParticipants, deliveryDesc, materialDesc, checklist, mottagningsSignatures]);
  ...
  const [, setShowCreateByggdelMoment] = useState(false);
  const [, setNewByggdelMomentName] = useState('');
  ...
  }, [showCreateByggdelMallModal, createMallKeyboardHeight, DEFAULT_CREATE_MALL_KEYBOARD_HEIGHT_IOS]);
  ...
  const applySelectedCategoriesToChecklist = useCallback((categoryFlags) => {
    try {
      const cfg = (Array.isArray(checklistConfig) ? checklistConfig : []);
      const flags = Array.isArray(categoryFlags) ? categoryFlags : [];
      const current = Array.isArray(checklistRef.current) ? checklistRef.current : [];
      ...
      const prevChecklist = Array.isArray(checklistRef.current) ? checklistRef.current : [];
      if (!shallowEqual(newChecklist, prevChecklist)) {
        setChecklist(newChecklist);
      }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const currentChecklist = Array.isArray(checklistRef.current) ? checklistRef.current : [];
        if (!currentChecklist || currentChecklist.length === 0) {
          applySelectedCategoriesToChecklist(flags);
        }
      } catch(_e) {}
    })();
  }, [controlType, projectId, checklistConfig, initialValues, applySelectedCategoriesToChecklist]);
  ...
  }, [controlType, projectId, defaultByggdelMomentsByGroup]);
              ...
              (async () => {
                try {
                  ...
                  try {
                    ...
                    // ...existing code...
                  } catch(_e) { try { console.warn('[BaseControlForm] persist after attach failed', _e); } catch(_e2) {} }
                } catch(_e) { try { console.warn('[BaseControlForm] persist after attach failed', _e); } catch(_e2) {} }
              })();
            }
          }
        ...
        try {
          ...
          (async () => {
            try {
              ...
              } catch(_e) { try { console.warn('[BaseControlForm] persist after uri add failed', _e); } catch(_e2) {} }
            } catch(e) {
