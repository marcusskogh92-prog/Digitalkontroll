/**
 * Enkel modal för att skapa en ny kategori i kategoriregistret.
 * Fält: Kategori (benämning), Beskrivning (valfritt).
 * Samma stil som Lägg till konto / Lägg till byggdel (mörk header, raka hörn).
 */
...
export default function AddKategoriModal({
  ...
  const handleSave = async () => {
    const n = (name ?? '').trim();
    if (!n) {
      setError('Ange kategori (benämning).');
      return;
    }
    setError('');
    try {
  ...
  return (
    ...
    >
      <Pressable style={[styles.overlay, overlayStyle]} onPress={onClose}>
        <Pressable
          style={[styles.box, boxStyle, error ? { minHeight: 320 } : null]}
        ...
        >
          ...
          >
            <Text style={styles.title} numberOfLines={1}>Lägg till kategori</Text>
          ...
          >
            <View style={styles.content}>
              ...
              <Text style={styles.label}>Kategori (benämning)</Text>
              <TextInput
                value={name}
