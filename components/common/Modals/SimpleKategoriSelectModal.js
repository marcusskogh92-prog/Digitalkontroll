/**
 * Enkel modal för att välja kategorier (t.ex. för leverantörsformuläret).
...
export default function SimpleKategoriSelectModal({
  visible,
  onClose,
  categories = [],
  selectedIds = [],
  ...
  const filteredCategories = useMemo(() => {
    ...
    return list.filter((item) => {
      ...
      const id = getCategoryValue(item);
      return label.toLowerCase().includes(q) || String(id || '').toLowerCase().includes(q);
    });
    ...
    >
      <Pressable style={[styles.overlay, overlayStyle]} onPress={onClose}>
        <Pressable style={[styles.box, boxStyle]} onPress={(e) => e.stopPropagation()}>
          ...
          >
            <Text style={styles.title} numberOfLines={1}>Välj kategorier</Text>
            <TouchableOpacity
              onPress={onClose}
            ...
            <ScrollView style={styles.scroll} keyboardShouldPersistTaps="handled">
              ...
              {(!categories || categories.length === 0) ? (
                <View style={[styles.row, { borderBottomWidth: 0 }]}>
                  <Text style={[styles.rowLabel, { color: '#94a3b8' }]}>Inga kategorier i företagets register.</Text>
                </View>
