/**
 * Kontaktpersoner för en kund – synkas till Kontaktregistret.
 */
...
import type { Contact, Customer } from './kunderService';
import { addContactToCustomer, removeContactFromCustomer } from './kunderService';
...
export default function KundContacts({
  companyId,
  customer,
  allContacts,
  onContactsChange,
}: KundContactsProps): React.ReactElement {
  ...
  const handleAdd = async (): Promise<void> => {
    ...
    setSaving(true);
    try {
      await addContactToCustomer(companyId, customer, {
        name: n,
  ...
  const handleRemove = async (contactId: string): Promise<void> => {
    ...
    const ok =
      Platform.OS === 'web'
        ? window.confirm('Ta bort kontakten från kunden?')
        : await new Promise<boolean>((resolve) => {
            Alert.alert(
              'Ta bort kontakt',
              'Koppla bort kontakten från denna kund? Kontakten finns kvar i Kontaktregistret.',
              [
  ...
  if (!customer) return <View style={styles.section} />;
