/**
 * Generic hook for calculating progress (works for all phases)
 */

import { useState, useEffect, useCallback } from 'react';
import { getAllCompletions } from '../kalkylskede/services/kalkylskedeService';
...
export function usePhaseProgress(companyId, projectId, phaseKey, navigation) {
  ...
  const loadCompletions = useCallback(async () => {
    ...
    try {
      ...
      console.error('[usePhaseProgress] Error loading completions:', error);
    } finally {
      setIsLoading(false);
    }
  }, [companyId, projectId]);
