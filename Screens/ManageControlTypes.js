...
export default function ManageControlTypes({ route, navigation }) {
  ...
  if (Platform.OS === 'web') {
    ...
    const showFoldersColumn = false; // Mittenpanelen (Mappar) är borttagen – Mallar visar alla mallar för vald kontrolltyp
  ...
  useEffect(() => {
    ...
    const templatesForSelectedFolder = useMemo(() => {
      // If folder column is hidden or folders are disabled, show all templates regardless of folder
      if (!showFoldersColumn || !foldersEnabled) {
    ...
    }, [templatesForSelectedType, selectedFolderId, templateSearch, foldersEnabled, showFoldersColumn]);
